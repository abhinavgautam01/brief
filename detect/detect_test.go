...
func TestCargoWorkspaceMemberLockfileDependencies(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, dir, "Cargo.toml", `[workspace]
members = ["crates/*"]
`)
...
serde = "1"
`)
	writeFile(t, dir, "crates/app/Cargo.lock", `# This file is automatically @generated by Cargo.
version = 4
...
`)
	...
	wantManifest := brief.ManifestInfo{
		Ecosystem: "cargo",
		Path:      "crates/app/Cargo.lock",
		Kind:      "lockfile",
	}
	if !slices.Contains(r.Manifests, wantManifest) {
		t.Errorf("manifests should contain %+v, got %+v", wantManifest, r.Manifests)
	}
	if !slices.ContainsFunc(r.Dependencies, func(dep brief.DepInfo) bool {
		return dep.Name == "serde_core" && strings.HasPrefix(dep.PURL, "pkg:cargo/serde_core@") &&
			dep.Version == "1.0.219" && !dep.Direct
	}) {
		t.Errorf("dependencies should contain the pinned transitive serde_core package, got %+v", r.Dependencies)
	}
}
...
`)
	...
	r := runOn(t, dir)
	for _, want := range []brief.ManifestInfo{
		{Ecosystem: "cargo", Path: "Cargo.toml", Kind: "manifest"},
		{Ecosystem: "cargo", Path: "Cargo.lock", Kind: "lockfile"},
	} {
		if !slices.Contains(r.Manifests, want) {
			t.Errorf("manifests should contain %+v, got %+v", want, r.Manifests)
		}
	}
	if !slices.ContainsFunc(r.Dependencies, func(dep brief.DepInfo) bool {
		return dep.PURL == "pkg:cargo/serde" && dep.Version == "1" && dep.Direct
	}) {
		t.Errorf("dependencies should contain the direct serde constraint, got %+v", r.Dependencies)
	}
	if !slices.ContainsFunc(r.Dependencies, func(dep brief.DepInfo) bool {
		return dep.Name == "serde_core" && strings.HasPrefix(dep.PURL, "pkg:cargo/serde_core@") &&
			dep.Version == "1.0.219" && !dep.Direct
